...
class First extends Component {
    ...
    // 获取公告列表
    async _getFirstList(offset = 0) {
        const params = {
            offset,
        }
        const result = await API.getFirstList({params});
        const { status, msg, data } = result;
        if (status === 200) {
            const { total: total_num, entitys: firstinfos } = data;
            this.setState({
                total_num,
                firstinfos,
            })
    ...
    // 分页保持函数
    _keepPage() {
        const { firstinfos, currentPage } = this.state;
        if (firstinfos.length === 1 && currentPage > 1) {
    ...
    handleNoClick = async (id, e) => {
        ...
        const result = await API.modifyFirst({data});
    ...
    render() {
        const { total_num, firstinfos, currentPage } = this.state;
        const PanelHeader = (props) => {
            const { id, applyTime, typeId } = props.firstinfos;
            return (
        ...
        const PanelCentent = (props) => {
            const { studentName, studentNum, typeId, fatherName, fatherPhone, fatherProfe, motherName, motherPhone, motherProfe, income, money, reason, imageAdd } = props.firstinfos;
        ...
        return(
            <div className='first-manage-container'>
                <div className='first-list-container'>
                    <Row className='header'>材料初审</Row>
                    ...
                    </Row>
                    {
                        ...
                        <Collapse>
                        {
                            firstinfos.map((item, i) => {
                                return(
                                    <Panel header={<PanelHeader firstinfos={item}/>} key={i} style={{backgroundColor: '#fff'}}>
                                        <PanelCentent firstinfos={item}/>
                                    </Panel>
                                )
                            })
                        }
